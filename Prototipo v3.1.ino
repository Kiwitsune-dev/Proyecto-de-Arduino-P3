...
*/
void alarmaIntermitente(int pin, int timer){
  pinMode(pin, OUTPUT);
...
void loop() {
  ...
  /*Detector de proximidad y Alarma sonora*/
  int infrarrojoEstado = digitalRead(infrarrojo);

  if(infrarrojoEstado == HIGH){
    //alarmaIntermitente(buzzerPin, 500);
    cabeceo.write(45);
  }

  if (infrarrojoEstado == LOW) {
    cabeceo.write(0);
  }
